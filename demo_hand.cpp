...
int main() {
    ...
    int32_t nq_freq = nq - nq_test;// 前9/10的数据用来统计frequency
    ...
    for(int i=0;i<nq_test;i++)
    {
        ...
        for(int k=0; k<32; k++)//计算LUT
        {
            for(int m=0; m<256; m++)
            {
                ...
                for(int n=0; n<4; n++)
                {
                    LUT[k*256 + m] += (int32_t)(xq_tmp[i*d + q_idx] * codebook[k*256*4 + m *4 +n]);
                    q_idx = q_idx + 1;
                }
        ...
        for(int j = 0 ; j< nprobe;j++)
        {
            ...
            for(int k=0;k<d;k++)
            {
                q_c += (int32_t)(xq_tmp[i*d+k] * centroid_q[k]);
            }
            // printf("q_c: %d \n", q_c);
            index->pq.compute_inner_prod_table(&xq_tmp[i*d], LUT_temp);
            for(int k=0; k<codes[idx_q[i*nprobe+j]].size(); k+=32)
            {
                int32_t res = 0;
                for(int m=0; m<32; m++)
                {
                ...
                res += q_c;
                I_temp[i].push_back(std::make_pair(res, ids[idx_q[i*nprobe+j]][k/32]));
            }
        }
